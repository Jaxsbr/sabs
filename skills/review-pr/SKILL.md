---
name: review-pr
description: "Reviews a GitHub pull request for correctness, security, design, and maintainability — posts inline comments or displays locally"
context: fork
allowed-tools: "Bash(gh *) Bash(git *) Bash(jq *) Bash(curl *) Read(*) Grep(*) Glob(*)"
---
...
|----------|-------------|---------|
| **Critical** | Bugs, security holes, data loss, broken logic, missing error handling that will bite in production | Race condition, SQL injection, unchecked null on a critical path |
| **Concern** | Design issues, maintainability risks, unclear intent, missing tests for important behaviour | Tight coupling, duplicated logic across files, ambiguous naming on a public API |
