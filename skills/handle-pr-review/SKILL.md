...
description: "Triages PR review comments — fixes valid issues, challenges incorrect feedback, resolves threads, and posts summary"
allowed-tools: "Bash(gh *) Bash(git *) Bash(npm *) Bash(npx *) Bash(make *) Bash(cargo *) Bash(python *) Bash(jq *) Bash(curl *) Read(*) Write(*) Edit(*) Grep(*) Glob(*)"
---
...
Resolve the short commit SHA first, then post. Use `gh pr comment` with `-f` body or unquoted heredoc:
